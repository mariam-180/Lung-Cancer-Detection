...
export default function DoctorProfile() {
  ...
  async function authRequest(requestFn) {
    try {
      ...
      if (err?.response?.status === 401) {
        const newToken = await refreshAccessToken();
        if (newToken) return await requestFn(newToken);
      }
      throw err;
    }
  }

  function normalizeDoctor(data) {
    return {
      ...data,
      fullName: data?.fullName || data?.name || '',
      ...
      averageRating: data?.averageRating || data?.rating || null,
      totalReviews: data?.totalReviews || data?.reviews || null,
    };
  }

  function populateForm(normalized) {
    setForm({
      fullName: normalized.fullName,
      specialty: normalized.specialization,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const normalized = await fetchProfile();
      updateUser({ fullName: normalized.fullName });
      ...
      <div className={Style.inner}>
        ...
        <div className={Style.leftCol}>
          ...
          <div className={Style.statRow}>
            ...
            <div className={Style.statDivider}></div>
            <div className={Style.stat}>
              <strong>{doctor?.totalReviews ?? '—'}</strong>
              <span>Reviews</span>
            </div>
          </div>
          ...
          <div className={Style.leftSection}>
            <div className={Style.leftSectionHeader}>
              <div className={Style.leftSectionIcon}><i className="fa-solid fa-list-check"></i></div>
              <h3 className={Style.leftSectionTitle}>Services</h3>
            </div>
            <div className={Style.servicesList}>
              <div className={Style.serviceRow}>
                <div className={Style.serviceLeft}>
        ...
        <div className={Style.rightCol}>
          ...
          {activeTab === 'profile' && (
            <>
              ...
              <div className={Style.section}>
                <div className={Style.sectionHeader}>
                  <div className={Style.sectionIcon}><i className="fa-solid fa-clipboard-list"></i></div>
                  <h3 className={Style.sectionTitle}>Professional Details</h3>
                </div>

                <div className={Style.compactInfoGrid}>
                  <div className={Style.compactInfoItem}>
                    <div className={Style.compactIcon}><i className="fa-solid fa-hospital"></i></div>
                    <div className={Style.compactContent}>
                      <span className={Style.compactLabel}>Hospital</span>
                      <span className={Style.compactValue}>{doctor?.hospitalName || 'N/A'}</span>
                    </div>
                  </div>

                  <div className={Style.compactInfoItem}>
                    <div className={Style.compactIcon}><i className="fa-solid fa-phone"></i></div>
                    <div className={Style.compactContent}>
                      <span className={Style.compactLabel}>Phone</span>
                      <span className={Style.compactValue}>{doctor?.phoneNumber || 'N/A'}</span>
                    </div>
                  </div>

                  <div className={Style.compactInfoItem}>
                    <div className={Style.compactIcon}><i className="fa-solid fa-id-badge"></i></div>
                    <div className={Style.compactContent}>
                      <span className={Style.compactLabel}>License</span>
                      <span className={Style.compactValue}>{doctor?.licenseNumber || 'N/A'}</span>
                    </div>
                  </div>

                  <div className={Style.compactInfoItem}>
                    <div className={Style.compactIcon}><i className="fa-solid fa-star"></i></div>
                    <div className={Style.compactContent}>
                      <span className={Style.compactLabel}>Rating</span>
                      <span className={`${Style.compactValue} ${Style.ratingBadge}`}>
                        {doctor?.averageRating ? `${doctor.averageRating} / 5` : 'N/A'}
                      </span>
                    </div>
                  </div>

                  <div className={Style.compactInfoItem}>
                    <div className={Style.compactIcon}><i className="fa-solid fa-comments"></i></div>
                    <div className={Style.compactContent}>
                      <span className={Style.compactLabel}>Reviews</span>
                      <span className={Style.compactValue}>{doctor?.totalReviews || '0'}</span>
                    </div>
                  </div>

                  <div className={Style.compactInfoItem}>
                    <div className={Style.compactIcon}><i className="fa-solid fa-check-circle"></i></div>
                    <div className={Style.compactContent}>
                      <span className={Style.compactLabel}>Status</span>
                      <span className={`${Style.compactValue} ${Style.statusBadge} ${
                        doctor?.approvalStatus === 'Approved' ? Style.statusApproved :
                        doctor?.approvalStatus === 'Pending' ? Style.statusPending :
                        Style.statusRejected
                      }`}>
                        {doctor?.approvalStatus || 'N/A'}
                      </span>
                    </div>
                  </div>

                  <div className={Style.compactInfoItem}>
                    <div className={Style.compactIcon}><i className="fa-solid fa-calendar-plus"></i></div>
                    <div className={Style.compactContent}>
                      <span className={Style.compactLabel}>Joined</span>
                      <span className={Style.compactValue}>{formatDate(doctor?.createdAt)}</span>
                    </div>
                  </div>

                  <div className={Style.compactInfoItem}>
                    <div className={Style.compactIcon}><i className="fa-solid fa-briefcase"></i></div>
                    <div className={Style.compactContent}>
                      <span className={Style.compactLabel}>Experience</span>
                      <span className={Style.compactValue}>
          ...
          {activeTab === 'edit' && (
            <div className={Style.section}>
              ...
              <div className={Style.formGrid}>
                ...
                {/* ── Email: disabled, visually distinct ── */}
                <div className={Style.formGroup}>
                ...
                <div className={Style.formGroup}>
                  <label className={Style.formLabel}><i className="fa-solid fa-hospital"></i> Hospital</label>
                  <input
                    type="text"
                    name="hospitalName"
                ...
                <div className={Style.formGroup}>
                  <label className={Style.formLabel}><i className="fa-solid fa-briefcase"></i> Years of Experience</label>
                  <input
                    type="number"
              ...
              <div className={Style.formActions}>
                <button
                  ...
                  onClick={() => { setActiveTab('profile'); setSaveError(''); setSaveSuccess(false); }}
                  disabled={saving}
                >
                  Cancel
                </button>
          ...
          {activeTab === 'password' && (
            <div className={Style.section}>
              ...
              <div className={Style.formActions}>
                <button
                  className={Style.cancelBtn}
                  onClick={() => { setActiveTab('profile'); setPasswordError(''); setPasswordSuccess(false); }}
